...
const { validateVideoEdits } = require("./lib/validate");
...
module.exports = async (req, res) => {
  ...
  const validationError = validateVideoEdits(payload);
  if (validationError) {
    res.status(400).json({ error: "invalid_payload", message: validationError });
    return;
  }

  const result = await writeJsonFile("video-edits.json", payload, "admin: update video-edits.json");
