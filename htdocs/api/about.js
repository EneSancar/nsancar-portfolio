...
const { validateAbout } = require("./lib/validate");
...
module.exports = async (req, res) => {
  ...
  const validationError = validateAbout(payload);
  if (validationError) {
    res.status(400).json({ error: "invalid_payload", message: validationError });
    return;
  }

  const result = await writeJsonFile("about.json", payload, "admin: update about.json");
