...
const { validateActivities } = require("./lib/validate");
...
module.exports = async (req, res) => {
  ...
  const validationError = validateActivities(payload);
  if (validationError) {
    res.status(400).json({ error: "invalid_payload", message: validationError });
    return;
  }

  const result = await writeJsonFile("activities.json", payload, "admin: update activities.json");
